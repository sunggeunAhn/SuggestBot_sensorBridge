...
public class SpeechService extends Service {
        ...
        public void onNext(StreamingRecognizeResponse response) {
            ...
            if (text != null) {
                for (Listener listener : mListeners) {
                    listener.onSpeechRecognized(text, confidence, isFinal);
                }
            }
        }

        @Override
        public void onError(Throwable t) { Log.e(TAG, "Error calling the API.", t); }

        @Override
        public void onCompleted() { Log.i(TAG, "API completed."); }
    };
    ...
    private final StreamObserver<RecognizeResponse> mFileResponseObserver
        ...
        public void onNext(RecognizeResponse response) {
            ...
            if (text != null) {
                for (Listener listener : mListeners) {
                    listener.onSpeechRecognized(text, confidence, true);
                }
            }
        }
    ...
    private class SpeechBinder extends Binder {
        SpeechService getService() { return SpeechService.this; }
    }
    ...
    private static class GoogleCredentialsInterceptor implements ClientInterceptor {
        ...
        private URI removePort(URI uri) throws StatusException {
            try {
                return new URI(
                        uri.getScheme(),uri.getUserInfo(), uri.getHost(), -1 /* port */,
